...
Mock.mock('records_api/records/list', 'post', option =>{
  ...
  if (Book_id) {  
    arr = arr.filter(item => (item.Book_id+"").includes(Book_id)||Object.is(item.Book_id,Book_id));
  }
  if (Book_name) {
    arr = arr.filter(item => item.Book_name.includes(Book_name));
  }
  if (User_id) {
    arr = arr.filter(item =>  (item.User_id+"").includes(User_id)||Object.is(item.User_id,User_id));
  }
  ...
  Mock.mock("records_api/records/add", "post", function(option) { 
    ...
    var obj = newObj({
      Book_id, Book_name, Author, User_id,Penalty,
      Borrow_date: today,
      Exp_return_date:moment(today).add(30, 'day').format('YYYY-MM-DD'),
      Act_return_date:''
    });
...
Mock.mock("records_api/records/update", "post", function(option) {
  ...
  let { Record_id, Book_name, Author, User_id,Penalty
  } = JSON.parse(option.body); 

  Object.assign(arr[Record_id], {
    Book_name, Author, User_id,Penalty
  });
